...
<br/>As a result, CNNs cannot model the distribution of values in regions, a factor that is essential for texture analysis [(Humeau-Heurtier, 1993)](https://ieeexplore.ieee.org/abstract/document/8600329). Deep architectures require
more layers resulting in more parameters to characterize
the spatial distribution of features in a convolutional neural
network (CNN), as opposed to using a histogram directly. 

As a result, CNNs fail to characterize the spatial distribution of features and require more layers to capture the distributions of values in a region. 
